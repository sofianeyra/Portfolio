...
export function PortfolioHome() {
  ...
  return <main className="overflow-hidden">
    <nav className="sticky top-0 z-20 border-b border-white/10 bg-[#282631]/95 text-[#f5f4f8] backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between px-4 py-4 md:px-8">
        <Link href="#top" className="inline-flex items-center transition-opacity hover:opacity-75" aria-label="Ir al inicio">
          <Image src="/images/logo.png" alt="Sofia Neyra" width={58} height={27} priority className="h-auto w-[52px]" />
        </Link>
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => setLocale(locale === "es" ? "en" : "es")} className="rounded-full border border-white/20 px-3 py-1.5 text-xs font-medium transition hover:bg-white/10 active:scale-[0.98]" aria-label="Change language">{locale.toUpperCase()}</button><button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="grid size-8 place-items-center rounded-full border border-white/20 transition hover:bg-white/10 active:scale-[0.98]" aria-label="Toggle color theme">{theme === "dark" ? <Sun size={16} weight="light" /> : <Moon size={16} weight="light" />}</button></div></div></nav>
